...
Uses only persisted meeting/member snapshots. It never marks an approval or
signature completed unless the related transaction evidence says so.
"""
...
def build_committee_meeting_pdf(*, company: dict, meeting: dict, members: list[dict]) -> bytes:
    ...
    normal = ParagraphStyle(
        "normal-tr", parent=styles["BodyText"], fontName=_FONT, fontSize=8.5,
        leading=11, textColor=colors.HexColor("#1f2937"),
    )
    small = ParagraphStyle("small-tr", parent=normal, fontSize=7, leading=9)
    heading = ParagraphStyle(
        "heading-tr", parent=normal, fontName=_FONT_B, fontSize=11, leading=14,
        textColor=colors.HexColor("#0f3d63"), spaceBefore=6, spaceAfter=5,
    )
    ...
    sig_name = ParagraphStyle("sig-name-tr", parent=center, fontName=_FONT_B, fontSize=8.5)
    ...
    def footer(canvas, doc):
        ...
        verification = str(meeting.get("pdf_sha256") or "")[:16]
        footer_text = (
            f"İSG Kurulu Toplantı Tutanağı | {company.get('name') or '—'} | "
            f"Belge No: {meeting.get('document_no') or '—'} | Rev: {meeting.get('revision_no') or '00'} | "
            f"Sürüm: {meeting.get('document_version') or 1}"
        )
        if verification:
            footer_text += f" | Doğrulama: {verification}…"
        canvas.drawString(margin, 8 * mm, footer_text[:145])
    ...
    header = Table(
        [[
            ...
            Table([
                ...
                [_p("Belge sürümü", label), _p(meeting.get("document_version") or 1, small)],
    ...
    info_rows = [
        ...
        ["Toplantı durumu", meeting.get("status") or "draft", "Dijital onay", meeting.get("approval_status") or "draft"],
        ["E-imza durumu", meeting.get("signature_status") or "not_signed", "Güncel onay adımı", meeting.get("approval_current_step") or "—"],
        ["Sonraki toplantı", meeting.get("next_meeting_date"), "Üye sayısı", len(members)],
    ]
    info = Table(
        [[_p(cell, label if index % 2 == 0 else normal) for index, cell in enumerate(row)] for row in info_rows],
        colWidths=[31 * mm, 57 * mm, 31 * mm, 57 * mm],
    )
    ...
    participant_data = [[
        _p("#", label), _p("Ad Soyad", label), _p("Görevi / Unvanı", label),
        _p("Kurul Rolü", label), _p("Katılım", label), _p("Onay / İmza", label),
    ]]
    seen = set()
    unique_members = []
    ...
    for index, member in enumerate(unique_members, 1):
        approval_status = member.get("approval_status") or "Onay kaydı yok"
        signature_status = member.get("signature_status") or "İmzalanmadı"
        participant_data.append([
            ...
            _p(f"Onay: {approval_status}\nİmza: {signature_status}", small),
        ])
    participants = Table(
        participant_data,
        colWidths=[8 * mm, 38 * mm, 39 * mm, 34 * mm, 23 * mm, 34 * mm],
        repeatRows=1,
    )
    ...
    story.append(Paragraph("Onay ve İmza Alanları", heading))
    story.append(_p(
        "Boş alanlar ıslak imza için ayrılmıştır. Dijital onay, elektronik imza yerine geçmez. "
        "‘Elektronik olarak imzalandı’ ifadesi yalnız doğrulanmış imza artefaktı mevcutsa gösterilir.",
        small,
    ))
    ...
    for member in unique_members:
        approval_status = member.get("approval_status") or "Onay kaydı yok"
        approved_at = member.get("approved_at") or "—"
        signature_status = member.get("signature_status") or "İmzalanmadı"
        signed_at = member.get("signed_at") or "—"
        card = Table([
            [_p(member.get("full_name"), sig_name)],
            [_p(member.get("job_title") or member.get("professional_role"), center)],
            [_p(member.get("role_label") or member.get("role_code"), center)],
            [Spacer(1, 18 * mm)],
            [_p(f"Dijital Onay: {approval_status}", small)],
            [_p(f"Onay Tarihi: {approved_at}", small)],
            [_p(f"E-İmza Durumu: {signature_status}", small)],
            [_p(f"İmza Tarihi: {signed_at}", small)],
        ], colWidths=[54 * mm], rowHeights=[None, None, None, 18 * mm, None, None, None, None])
        card.setStyle(TableStyle([
            ("BOX", (0, 0), (-1, -1), 0.6, colors.HexColor("#94a3b8")),
            ("BACKGROUND", (0, 0), (-1, 2), colors.HexColor("#f8fafc")),
            ("BACKGROUND", (0, 4), (-1, 5), colors.HexColor("#f0f9ff")),
            ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
            ("ALIGN", (0, 0), (-1, 3), "CENTER"),
            ("PADDING", (0, 0), (-1, -1), 4),
        ]))
        signature_cards.append(card)
    for index in range(0, len(signature_cards), 3):
        row = signature_cards[index:index + 3]
        while len(row) < 3:
