...
from app.services.training_lifecycle_v2 import WORK_SPECIFIC_HOURS, applies_to_created_at, training_kind
...
def _information_refresh_curriculum(training, pdf_module) -> dict:
    ...
    return {
        ...
        "purpose": "Altı aydan fazla süreyle işten uzak kalan çalışanın işe başlamadan önce, işe ve işyerine özgü güncel riskler ve korunma tedbirleri hakkında bilgisini yenilemek.",
        "legal_basis": "Çalışanların İş Sağlığı ve Güvenliği Eğitimlerinin Usul ve Esasları Hakkında Yönetmeliğin bilgi yenileme eğitimi ve Ek-1 dördüncü konu başlığı hükümleri kapsamında düzenlenmiştir.",
        "disclaimer": "Bu kayıt Bilgi Yenileme Eğitimi tutanağıdır; düzenli Temel İSG tekrar eğitimi ve Temel İSG katılım belgesi yerine geçmez.",
...
def install_training_lifecycle_v2_content_guards() -> dict[str, str]:
    global _original_exam_builder, _original_certificate_builder, _original_curriculum
    global _original_presentation_readiness, _original_pilot_access
    ...
    if getattr(current_exam, "_premium_training_lifecycle_v2", False):
        ...
        def premium_exam_builder(*, company_name: str, training, db, created_by_id: int) -> bytes:
            kind = _record_only_kind(training)
            if kind:
                raise ValueError(f"{_label(kind)} için 20 soruluk Temel İSG sınavı oluşturulmaz. Bu kayıt türünde katılım/tutanak akışını kullanın.")
            return _original_exam_builder(company_name=company_name, training=training, db=db, created_by_id=created_by_id)
    ...
    if getattr(current_certificate, "_premium_record_only_certificate_guard", False):
        ...
        def premium_certificate_builder(*, company_name: str, training, employees: dict) -> bytes:
            kind = _record_only_kind(training)
            if kind:
                raise ValueError(f"{_label(kind)} için Temel İSG katılım belgesi oluşturulmaz. Bu eğitim türünün Katılım/Tutanak PDF kaydını kullanın.")
            return _original_certificate_builder(company_name=company_name, training=training, employees=employees)
    ...
    if getattr(current_readiness, "_premium_training_lifecycle_v2", False):
        ...
        def premium_presentation_readiness(db, *, training):
            ...
            if not kind:
                return payload
            label = _label(kind)
            check = {"code": "training_lifecycle_scope", "label": "Eğitim türü", "ok": False, "detail": f"{label}, Temel İSG sunum/sınav akışından ayrıdır; yanlış temel eğitim içeriği üretilmez."}
            checks = list(payload.get("checks") or [])
            if not any(item.get("code") == check["code"] for item in checks): checks.append(check)
            blockers = list(payload.get("blockers") or [])
            if not any(item.get("code") == check["code"] for item in blockers): blockers.append({"code": check["code"], "detail": check["detail"]})
            payload.update({"visible": False, "generation_allowed": False, "checks": checks, "blockers": blockers, "next_action": f"{label} için Katılım/Tutanak akışını kullanın; Temel İSG sunumu bu kayıt türünde üretilmez."})
            return payload
    ...
    if getattr(current_access, "_premium_training_lifecycle_v2", False):
        ...
        def premium_pilot_access(training) -> None:
            kind = _record_only_kind(training)
            if kind:
                raise HTTPException(409, {"code": "training_type_not_supported", "message": f"{_label(kind)} için Temel İSG sunumu oluşturulmaz. Katılım/Tutanak akışını kullanın.", "core_training_unaffected": True})
