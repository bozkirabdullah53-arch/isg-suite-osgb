...
Work Start and Information Refresh are deliberately NOT Basic İSG certificate
flows. They still need a trustworthy attendance/finalization path. This module
wraps the existing completion service only for post-cutover record-only rows and
delegates every other training to the existing implementation.
"""
...
def record_only_preflight(training) -> dict[str, Any]:
    ...
    finalized = status == TrainingStatus.COMPLETED.value and bool(
        getattr(training, "attendance_verified", False)
    )
