...
def _build_special_profile_manifest(*, training: TrainingSession, snapshot: TrainingNaceSnapshot, contract: dict[str, Any], profile_key: str, exam_readiness: dict[str, Any] | None) -> dict[str, Any]:
    ...
    exam_count = int(exam.get("count") or profile.get("exam_question_count") or 20)
    ...
    add("assessment", "Sınav ve uygulamalı değerlendirme", [exam_ref, profile_ref], [{"type":"exam_distribution","foundation":0,"work_specific":exam_count},{"type":"exam_workflow_unchanged","value":True}])
