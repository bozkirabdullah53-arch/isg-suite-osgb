...
Phase 9 and later additive curated-coverage patches are installed from this
already source-controlled startup hook so the main application bootstrap stays
unchanged. Every coverage patch is separately fail-closed and delegates to the
existing resolver chain while disabled.
"""
...
def install_phase8_generation_guard() -> dict[str, str]:
    ...
    from app.services.training_presentation_phase10_chemicals import install_training_presentation_phase10_chemicals

    phase9_status = install_training_presentation_phase9()
    phase9_scope_status = install_phase9_scope_guard()
    chemical_status = install_training_presentation_phase10_chemicals()
    ...
    if getattr(current, "_phase8_generation_guard_active", False):
        return {
            ...
            "chemical_pack": str(chemical_status.get("enabled", "false")),
        }
