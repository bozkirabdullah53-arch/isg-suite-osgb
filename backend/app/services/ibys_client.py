...
def validate_config() -> dict[str, Any]:
    ...
    key = (settings.ibys_api_key or "").strip()
    has_url = bool(url)
    has_key = bool(key)
...
def export_payload(*, osgb_id: int | None = None, dry_run: bool = True) -> dict[str, Any]:
    ...
    return {
        ...
        "items": [],
        "note": (
            "İBYS stub export — resmi API çağrısı yapılmadı. "
            "Kimlik bilgisi yoksa status=stub|missing_credentials."
            if st != "configured"
            else "İBYS kimlik bilgisi mevcut; canlı gönderim /integrations/ibys/live-send ile denenir."
        ),
    }
