...
def _work_start_state(
    employee: Employee,
    pairs: list[tuple[TrainingParticipant, TrainingSession]],
    *,
    cutover: date | None,
) -> dict[str, Any]:
    ...
    if cutover is None or hire is None or hire < cutover:
        return {
            "status": "historical",
            "label": "Tarihsel / takip dışı",
            "tone": "neutral",
            "training_id": None,
            "message": (
                "Premium İşe Başlama takibi bu çalışanın işe girişinden sonra devreye alınmadığı için "
                "geriye dönük eksik kaydı üretilmez."
            ),
        }

    work_start = [
        (participant, training)
        for participant, training in pairs
        if _status_value(training) != TrainingStatus.CANCELLED.value
        and training_kind(training.training_type, training.title) == "work_start"
    ]
    work_start.sort(
        key=lambda item: (
            item[1].end_date or item[1].start_date or date.min,
            item[1].id or 0,
        ),
        reverse=True,
    )
    ...
    if work_start:
        ...
        return {
            ...
            "tone": "warning",
            "training_id": training.id,
            "message": "İşe Başlama Eğitimi kaydı var; gerçek katılımın kesinleştirilmesi gerekiyor.",
        }
    return {
        "status": "missing",
        "label": "Eksik",
        "tone": "danger",
        "training_id": None,
        "message": "İşe Başlama Eğitimi kaydı bulunamadı; çalışan fiilen işe başlamadan önce planlayın.",
    }
...
def _basic_state(
    ...
    basic = [
        (participant, training)
        for participant, training in pairs
        if _status_value(training) != TrainingStatus.CANCELLED.value and _is_basic_training(training)
    ]
    basic.sort(
        key=lambda item: (
            item[1].end_date or item[1].start_date or date.min,
    ...
    evaluated = training_validity.evaluate_employee(
    ...
    if status == "ok":
        tone, label = "ok", "Geçerli"
    elif status == "due_soon":
        tone, label = "warning", "Yakında yenilenecek"
    elif status == "never" and days_left is not None and int(days_left) >= 0:
        tone, label = "warning", "İlk temel eğitim bekliyor"
    else:
        tone, label = "danger", "Gecikmiş / eksik"
    return {
        **evaluated,
        "tone": tone,
        "label": label,
...
def build_dashboard(db: Session, *, company_id: int) -> dict[str, Any]:
    ...
    pairs_by_employee: dict[int, list[tuple[TrainingParticipant, TrainingSession]]] = {
        employee_id: [] for employee_id in employee_ids
    }
    if employee_ids:
        pairs = db.execute(
            select(TrainingParticipant, TrainingSession)
            .join(TrainingSession, TrainingSession.id == TrainingParticipant.training_id)
            .where(
                TrainingSession.company_id == company_id,
                TrainingParticipant.employee_id.in_(employee_ids),
            )
        ).all()
        for participant, training in pairs:
            pairs_by_employee.setdefault(participant.employee_id, []).append((participant, training))

    cutover = _cutover_date()
    rows: list[dict[str, Any]] = []
    counts = {
        "work_start_missing": 0,
        "work_start_pending": 0,
        "work_start_ok": 0,
        "work_start_historical": 0,
        "basic_overdue": 0,
        "basic_waiting": 0,
        "basic_due_soon": 0,
        "basic_ok": 0,
    ...
    for employee in employees:
        ...
        counts[f"work_start_{work_start['status']}"] = counts.get(
            f"work_start_{work_start['status']}", 0
        ) + 1

        if basic["status"] == "ok":
            counts["basic_ok"] += 1
        elif basic["status"] == "due_soon":
            counts["basic_due_soon"] += 1
        elif basic["status"] == "never" and basic.get("days_left") is not None and int(basic["days_left"]) >= 0:
            counts["basic_waiting"] += 1
        else:
            counts["basic_overdue"] += 1
    ...
    actions = [
        {
            ...
            "severity": "danger",
            "count": counts["work_start_missing"],
        ...
        },
        {
            "code": "basic_due_soon",
            "severity": "warning",
            "count": counts["basic_due_soon"],
            "title": "Temel İSG yenilemesi yaklaşıyor",
            "instruction": "Süresi dolmadan yenileme eğitimini planlayın.",
            "target": "yenileme",
        },
        {
            "code": "basic_waiting",
            "severity": "warning",
            "count": counts["basic_waiting"],
            "title": "İlk temel eğitim bekliyor",
            "instruction": "İşe başladıktan sonra en kısa sürede ve en geç üç ay içinde tamamlayın.",
            "target": "temel",
        },
        {
            "code": "result_pending",
            "severity": "warning",
            "count": result_pending,
            "title": "Katılım / sonuç bekleyen eğitim",
            "instruction": "Gerçek katılımı ve gerekiyorsa sınav puanlarını girip sonuçları kesinleştirin.",
            "target": "kayitlar",
        },
        {
            "code": "planned_future",
            "severity": "info",
            "count": planned_future,
            "title": "Planlanmış yaklaşan eğitim",
            "instruction": "Eğitim tarihi ve katılımcı listesini kontrol edin.",
            "target": "kayitlar",
        },
    ]
    ...
    rows.sort(
        key=lambda row: (
            min(
                row_order.get(row["work_start"]["tone"], 9),
                row_order.get(row["basic"]["tone"], 9),
            ),
            row["full_name"] or "",
        )
    )
    ...
    return {
        "enabled": dashboard_active(),
        "version": DASHBOARD_VERSION,
        "company_id": company_id,
        "today": today.isoformat(),
        "work_start_tracking_from": cutover.isoformat() if cutover else None,
        "summary": {
        ...
        "safety": {
            "read_only": True,
            "automatic_training_completion": False,
            "historical_work_start_backfill_required": False,
        },
    }
