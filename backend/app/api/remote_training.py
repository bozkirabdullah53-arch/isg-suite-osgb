...
def _company_logo_for_remote_report(company_id: int) -> str | None:
    """Resolve the workplace's logo for the shared PDF/Excel report builders."""
    try:
        from app.services.remote_training_document_extension import (
            materialize_remote_company_logo_for_pdf,
        )

        return materialize_remote_company_logo_for_pdf(company_id)
    except Exception:
        # A missing optional logo must never make an otherwise valid report fail.
        logger.warning(
            "remote training company logo could not be prepared for report: company=%s",
            company_id,
            exc_info=True,
        )
        return None
...
def export_remote_certificate_records_xlsx(
    ...
    data = build_remote_training_status_xlsx(
        ...
        branch_name=branch.name if branch else None,
        logo_path=_company_logo_for_remote_report(company.id),
    )
