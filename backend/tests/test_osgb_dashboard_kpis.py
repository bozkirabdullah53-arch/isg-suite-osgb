"""0.9.112 â€” OSGB home finance overdue + active expiring-contract KPIs (uncapped counts)."""
...
def test_health_flag_osgb_home_kpis_v2(client):
    r = client.get("/health")
    assert r.status_code == 200
    body = r.json()
    assert body["version"] == "0.9.116"
    assert body["finance_overdue_alert"] == "dashboard-v2"
    assert body["osgb_home_kpis"] == "finance-contracts-v2"
...
def test_osgb_dashboard_finance_and_contract_kpis_uncapped(client):
    ...
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["kpi_version"] == "dashboard-finance-contracts-v2"
