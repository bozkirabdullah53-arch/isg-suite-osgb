...
def test_template_matches_user_sample_layout():
    ...
    assert ws.cell(TEMPLATE_DATA_START, 9).number_format == "DD.MM.YYYY"
...
def test_filled_template_roundtrip():
    ...
    ws.cell(TEMPLATE_DATA_START, 8, date(2024, 1, 15))
    ws.cell(TEMPLATE_DATA_START, 10, "Yok")
    ws.cell(TEMPLATE_DATA_START + 1, 2, "Ayşe Yılmaz")
    ws.cell(TEMPLATE_DATA_START + 1, 4, "Operatör")
    ws.cell(TEMPLATE_DATA_START + 1, 10, "Engelli")
    ws.cell(TEMPLATE_DATA_START + 1, 9, date(2024, 6, 30))
    buf = BytesIO()
    wb.save(buf)
    wb.close()
