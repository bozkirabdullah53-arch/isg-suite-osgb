...
function CatalogManagerPanel({companyId = '', branchId = '', onCompanyChange, onBranchChange, onPrepared, rollout = null, canEditContent = false, canEditSharedContent = false}) {
  ...
  function openCatalogVideoPicker(inputKey) {
    if (busy) return;
    const input = uploadInputRefs.current[inputKey];
    if (!input) {
      setError('Video seçme alanı hazır değil. Sayfayı yenileyip tekrar deneyin.');
      return;
    }
    input.click();
  }
