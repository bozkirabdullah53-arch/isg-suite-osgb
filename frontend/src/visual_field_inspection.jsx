...
export function VisualFieldInspectionPage() {
  ...
  async function createAndAnalyze(event) {
    ...
    try {
      ...
      setCurrent(fresh);
      setMessage(["failed", "not_configured"].includes(fresh.ai_status) ? "Denetim kaydedildi; AI taslağı oluşturulamadı. Fotoğraflar ve kayıt korunuyor." : "Denetim kaydedildi; AI analizi uzman onayı bekleyen taslak olarak kuyruğa alındı.");
        ...
        <aside className="visual-field-side">
          <section className="visual-card visual-current-card">
            ...
            {!current ? <p className="visual-muted">Fotoğraf yükleyip kaydettiğinizde analiz durumu, işaretli görseller ve uzman incelemesi burada görünür.</p> : <>
              <div className="visual-status-pills"><span>{formatStatus(current.status)}</span><span className={current.ai_status === "failed" ? "is-error" : current.ai_status === "not_configured" ? "is-warning" : ""}>AI: {formatStatus(current.ai_status)}</span></div>
              ...
              {["failed", "not_configured"].includes(current.ai_status) && <button type="button" className="visual-secondary-button full" onClick={retryAnalysis} disabled={busy}><RefreshCw size={15} /> AI analizini yeniden dene</button>}
