...
import './ohs-committee-member-modal.css';
...
export function OhsCommitteePage({user}) {
  ...
  const mandatoryCandidates = candidates.mandatory || [];
  const selectableCandidateCount = [...mandatoryCandidates, ...(candidates.other || [])].filter((candidate) => (
    !candidate.missing && !candidate.selected && !selectedKeys.has(candidate.identity_key)
  )).length;
  ...
  function CandidateCard({candidate}) {
    ...
    const isCurrent = Boolean(selectedCandidate && selectedCandidate.identity_key === candidate.identity_key);
    return <button type="button" className={`committee-v2-person ${disabled ? 'is-disabled' : ''} ${isCurrent ? 'is-current' : ''}`} disabled={disabled} onClick={() => selectCandidate(candidate)} aria-label={`${candidate.full_name} kişisini kurul üyesi olarak seç`} aria-pressed={isCurrent}>
      ...
      <span className="committee-v2-badges">{candidate.mandatory && <b className="is-mandatory">Zorunlu</b>}<b className={isCurrent ? 'is-current' : disabled ? 'is-selected' : 'is-eligible'}>{isCurrent ? 'Seçiminiz' : disabled ? 'Eklendi' : 'Seç'}</b></span>
    </button>;
  }
  ...
  return <div className="committee-v2-page">
    ...
    {open === 'member' && <AppModal
      title={<span className="committee-v2-dialog-title"><span className="committee-v2-dialog-icon"><ShieldCheck size={22} /></span><span><small>İSG Kurul Yönetimi</small><strong>Kurul Üyesi Seçimi</strong><em>{selectedCompany?.name || 'İşyeri seçilmedi'}</em></span></span>}
      close={() => {setOpen(''); setSelectedCandidate(null);}}
      wide
      className="committee-v2-member-modal"
    >
      <div className="committee-v2-picker">
        <section className="committee-v2-available">
          <div className="committee-v2-picker-intro">
            <span><strong>Aday personel</strong><small>Kurula eklemek istediğiniz kişiyi seçin.</small></span>
            <b>{selectableCandidateCount} uygun aday</b>
          </div>
          <div className="committee-v2-tools">
            <label className="committee-v2-search"><span>Personel ara</span><span className="committee-v2-control"><Search size={18} /><input aria-label="Personel ara" placeholder="Ad, görev veya departman" value={search} onChange={(event) => setSearch(event.target.value)} /></span></label>
            <label className="committee-v2-filter"><span>Göreve göre filtrele</span><select aria-label="Göreve göre filtrele" value={roleFilter} onChange={(event) => setRoleFilter(event.target.value)}><option value="">Tüm görevler</option><option value="calisan_temsilcisi">Çalışan Temsilcisi</option><option value="destek">Destek Elemanı</option><option value="diger">Diğer</option></select></label>
          </div>

          <div className="committee-v2-section-title"><span><strong>Zorunlu kurul üyeleri</strong><small>İşyeri görevlendirmelerinden otomatik getirilir.</small></span><b>{mandatoryCandidates.length}</b></div>
          <div className="committee-v2-candidate-list">
            {mandatoryCandidates.length ? mandatoryCandidates.map((candidate, index) => <CandidateCard key={candidate.identity_key || `missing-${index}`} candidate={candidate} />) : <div className="committee-v2-inline-empty"><ShieldCheck size={18} /><span><strong>Tanımlı zorunlu aday bulunmuyor</strong><small>İşyeri görevlendirmeleri tamamlandığında adaylar burada görünür.</small></span></div>}
          </div>

          <div className="committee-v2-section-title"><span><strong>Diğer kurul üyeleri</strong><small>Aktif işyeri personeli arasından seçim yapın.</small></span><b>{filteredOther.length}</b></div>
          <div className="committee-v2-candidate-list">
            {filteredOther.length ? filteredOther.map((candidate) => <CandidateCard key={candidate.identity_key} candidate={candidate} />) : <div className="committee-v2-empty small"><span className="committee-v2-empty-icon"><Users size={28} /></span><strong>{search || roleFilter ? 'Filtreye uygun personel bulunamadı' : 'Aktif personel bulunamadı'}</strong><span>{search || roleFilter ? 'Arama metnini veya görev filtresini değiştirin.' : 'Personel kayıtları eklendiğinde burada listelenecektir.'}</span>{(search || roleFilter) && <button type="button" className="secondary" onClick={() => {setSearch(''); setRoleFilter('');}}>Filtreleri temizle</button>}</div>}
          </div>
        </section>

        <aside className="committee-v2-selection">
          <div className="committee-v2-selection-head"><span>2</span><div><strong>Üyelik bilgileri</strong><small>Seçilen kişinin kurul görevini doğrulayın.</small></div></div>
          {selectedCandidate ? <form onSubmit={saveMember}>
            <div className="committee-v2-selected-person"><span className="committee-v2-avatar">{selectedCandidate.initials || initials(selectedCandidate.full_name)}</span><span><b>Seçilen personel</b><strong>{selectedCandidate.full_name}</strong><small>{selectedCandidate.job_title || selectedCandidate.professional_role || 'Görev bilgisi yok'}</small></span><UserCheck size={20} /></div>
            <Field label="Kurul görevi"><select required value={memberForm.role_code} onChange={(event) => setMemberForm({...memberForm, role_code: event.target.value})}>{(meta.roles || []).map((role) => <option key={role.code} value={role.code}>{role.label}</option>)}<option value="baskan">Kurul Başkanı</option><option value="sekreter">Kurul Sekreteri</option></select></Field>
            <Field label="Başlangıç tarihi" type="date" value={memberForm.start_date} onChange={(event) => setMemberForm({...memberForm, start_date: event.target.value})} />
            <Field label="Not"><textarea rows={4} placeholder="İsteğe bağlı açıklama ekleyin" value={memberForm.notes} onChange={(event) => setMemberForm({...memberForm, notes: event.target.value})} /></Field>
            <div className="form-actions"><button type="button" className="secondary" disabled={busy} onClick={() => setSelectedCandidate(null)}>Seçimi değiştir</button><button type="submit" disabled={busy}>{busy ? 'Kaydediliyor…' : <><Plus size={16} /> Kurula Ekle</>}</button></div>
          </form> : <div className="committee-v2-empty committee-v2-selection-empty"><span className="committee-v2-empty-icon"><UserCheck size={32} /></span><strong>Henüz bir kişi seçilmedi</strong><span>Soldaki aday listesinden bir personele tıklayın. Üyelik bilgileri bu alanda açılacaktır.</span><div className="committee-v2-mini-steps"><span><b>1</b> Personeli seçin</span><span><b>2</b> Görevi doğrulayın</span><span><b>3</b> Kurula ekleyin</span></div></div>}
        </aside>
      </div>
    </AppModal>}
