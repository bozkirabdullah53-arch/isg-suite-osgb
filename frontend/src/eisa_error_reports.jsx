...
export function EisaErrorReportsPage() {
  ...
  return (
    <div className="eisa-error-reports-page">
      <Page
      ...
      >
        ...
        <div className="table-wrap eisa-error-reports-table">
          <table>
            <thead>
              <tr>
                <th>Tarih</th>
            ...
            <tbody>
              {rows.length === 0 ? (
                ...
                <tr key={r.id}>
                  ...
                  <td data-label="HTTP">{r.http_status ? `${r.http_status} ${r.http_path || ''}` : (r.http_path || '—')}</td>
                  <td data-label="Adet">{r.occurrence_count || 1}</td>
                  <td data-label="Durum">{statusLabelsMap[r.status] || r.status}</td>
                  <td data-label="İşlem">
